...
let addCounterMock: jest.Mock<typeof addCounter>;
...
beforeAll(async () => {
    ...
    addCounterMock = service.addCounter as jest.Mock<typeof addCounter>;
    detailCounterMock = service.getCounterDetails as jest.Mock<typeof getCounterDetails>
    removeCounterMock = service.removeCounter as jest.Mock<typeof removeCounter>;
...
describe("counter.controller", () => {
    ...
    describe("GET /counters", () => {
        // test cases
            ...
            it("return 200 with paginated list of counters, stripped of internal id", async () => {
                const counters = [mockCounter(), mockCounter({ id: 2, uuid: "a1a1a1a1-15fb-4f2a-91d2-f72331d5d8d3", code: "C02" })];
                listCountersMock.mockResolvedValue({
                    data: counters,
                    total: 2,
                    page: 1,
                    limit: 10,
                ...
                expect(res.body.data[0].uuid).toBe(counters[0]!.uuid);
            })
    })
